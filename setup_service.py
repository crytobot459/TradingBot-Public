# --- START OF FILE setup_service.py ---

# setup_service.py (v3.3 - Tối ưu cho Server, tắt MAC Changer)
# Đặt file này vào thư mục gốc của Freqtrade và chạy với:
# - Cài đặt lần đầu: sudo python3 setup_service.py
# - Cài đặt lại sạch: sudo python3 setup_service.py --clean
import os
import shutil
import subprocess
import sys
...
ALL_SERVICES = [
    HEALTH_MONITOR_SERVICE_NAME,
    AUTOMATION_SERVICE_NAME,
    FREQTRADE_SERVICE_NAME,
]
...
def run_command(command: list, can_fail=False):
    """Chạy một lệnh hệ thống và kiểm tra lỗi."""
...
def check_system_dependencies():
    """Kiểm tra xem các gói Python cần thiết cho monitor đã được cài đặt trên hệ thống chưa."""
...
def create_and_write_service_file(
    service_name, description, user, working_dir, exec_start, **kwargs
):
    """Hàm trợ giúp để tạo nội dung và ghi file .service."""
    ...
    service_lines = [
        "[Service]",
        f"Type={kwargs.get('service_type', 'simple')}",
        f"User={user}",
        f"Group={group}",
    ]
    if working_dir:
...
def cleanup_services():
    """Dừng, vô hiệu hóa và xóa các file service cũ."""
...
def main():
    """Tự động hóa việc tạo và kích hoạt các dịch vụ systemd."""
    print("===============================================================")
    ...
    required_files = {
        "Freqtrade executable": freqtrade_executable,
        "Automation Manager script": automation_script,
        "Health Monitor script": health_monitor_script,
    }
    ...
    if not create_and_write_service_file(
        service_name=FREQTRADE_SERVICE_NAME,
    ...
    if not create_and_write_service_file(
        service_name=AUTOMATION_SERVICE_NAME,
        description="Freqtrade Automation Manager (Advisor)",
        user=username,
        working_dir=working_directory,
        exec_start_pre="/bin/sleep 20",
        exec_start=am_exec,
        after=f"After={FREQTRADE_SERVICE_NAME}.service",
        wants=f"Wants={FREQTRADE_SERVICE_NAME}.service",
    ):
        sys.exit(1)
    ...
    if not create_and_write_service_file(
        service_name=HEALTH_MONITOR_SERVICE_NAME,
        description="Freqtrade System Health Monitor & Watchdog",
        user="root",
        working_dir=working_directory,
        exec_start=hm_exec,
        after=f"After={AUTOMATION_SERVICE_NAME}.service",
        wants=f"Wants={AUTOMATION_SERVICE_NAME}.service",
    ):
        sys.exit(1)
...
# --- END OF FILE setup_service.py ---

# Ví dụ   cd ~/freqtrade
# 459459    sudo python3 setup_service.py
# ssh -i "/home/bot/Bản tải về/ssh-key-2025-10-31.key" ubuntu@140.245.52.164
#   sudo systemctl status automation_manager.service

# journalctl -u automation_manager -f

# journalctl -u freqtrade -f

# sudo python3 setup_service.py --clean
#  pkill -f freqtrade
#  rm /home/bot/freqtrade/tradesv3*.sqlite
# scp -i "/home/bot/Bản tải về/ssh-key-2025-10-31.key" /home/bot/freqtrade/ichimoku_scanner.py ubuntu@140.245.52.164:~/freqtrade/
# scp -i "/home/bot/Bản tải về/ssh-key-2025-10-31.key" /home/bot/freqtrade/automation_manager.py ubuntu@140.245.52.164:~/freqtrade/
# scp -i "/home/bot/Bản tải về/ssh-key-2025-10-31.key" /home/bot/freqtrade/config.json ubuntu@140.245.52.164:~/freqtrade/
# scp -i "/home/bot/Bản tải về/ssh-key-2025-10-31.key" /home/bot/freqtrade/user_data/strategies/external_signal_strategy.py ubuntu@140.245.52.164:~/freqtrade/user_data/strategies/
# scp -i "/home/bot/Bản tải về/ssh-key-2025-10-31.key" /home/bot/freqtrade/setup_service.py ubuntu@140.245.52.164:~/freqtrade/
# scp -i "/home/bot/Bản tải về/ssh-key-2025-10-31.key" /home/bot/freqtrade/health_monitor.py ubuntu@140.245.52.164:~/freqtrade/
# http://140.245.52.164:8080/trade
#  Bypass Code  1785 0916 3379
# 1. Commit và bỏ qua tất cả các bước kiểm tra
# git commit -m "Add and update all custom scripts and strategy (skip hooks)" --no-verify

# 2. Đẩy commit đó lên GitHub
#  git push